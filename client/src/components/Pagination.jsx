...
export default function Pagination({ data }) {
  ...
  return (
    <>
      ...
      <div className="bg-white w-full mb-5 p-5 text-lg shadow-xl">
        <p >Sort by</p>
      </div>
      <div className="grid grid-cols-4 gap-5">
        {itemFilter.map((item, index) => (
          <div
            ...
            key={index}
          >
            <img src={item.image_url} className="w-full h-60 object-cover" alt="Monas" />
            <div className="text-start p-4">
              <p className="text-start font-semibold">{item.name}</p>
              <p className="text-start font-thin">{item.place}</p>
              <p className="text-start font-thin">⭐{item.rating}({item.reviews})</p>
              <p className="text-start font-semibold">Rp.{item.price_per_night} / night</p>
            </div>
          </div>
        ))}
      </div>
