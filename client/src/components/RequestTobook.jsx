...
export default function RequestToBook() {
  ...
  const { dataPayment, handleCreate, isLoading: isCreating } = UsePaymentCreate(id);
  ...
  const taxAndFee = dataPayment 
    ? parseInt(dataPayment.taxAmount || 0) + parseInt(dataPayment.feeAmount || 0)
    : 0;
  ...
  return (
    <WrapperContent>
      ...
      <div className="sm:mx-0 mx-5 sm:mt-0 mt-4">
        ...
        <Link to="../bookings" className="hover:opacity-70 transition-opacity">
          <ArrowLeft
          ...
          <div className="bg-white text-gray-500 text-sm space-y-4 shadow-lg rounded-xl p-6">
            ...
            <div className="space-y-3 border-b-2 pb-4">
              <div className="flex justify-between items-start">
                <h2 className="font-semibold text-black text-lg">Trip Details</h2>
                <button
                  onClick={toggle}
              ...
              <div className="space-y-2">
                ...
                <p className="flex items-center gap-1">
                  ...
                  {dataPayment.numGuest} {dataPayment.numGuest === 1 ? 'Guest' : 'Guests'}
                </p>
                {dataPayment.duration && (
                  <p className="flex items-center gap-1">
                    <span className="font-medium text-black">Duration:</span>
                    {dataPayment.duration} {dataPayment.duration === 1 ? 'night' : 'nights'}
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3">
              <h2 className="text-lg text-black font-semibold">Guest Information</h2>
              <div className="space-y-2">
                <p className="flex flex-col sm:flex-row gap-1">
                  <span className="font-medium text-black">Name:</span>
                  <span className="capitalize">{user?.name || 'Not provided'}</span>
                </p>
                <p className="flex flex-col sm:flex-row gap-1">
        ...
        <aside className="space-y-5">
          <div className="bg-white rounded-xl text-gray-600 space-y-4 shadow-2xl p-6">
            <header className="space-y-1">
              <h2 className="font-semibold text-black text-lg">Payment Summary</h2>
            ...
            <section className="flex space-x-4 p-3 bg-gray-50 rounded-lg">
              <img
                src={`${import.meta.env.VITE_API_URL_LISTING_IMG}${dataPayment.listingId?.imgUrl?.[0]}`}
            ...
            <section className="text-sm py-4 border-t-2 space-y-3">
              ...
              <div className="flex justify-between items-center">
                <span className="text-gray-800 font-medium">Taxes & Fees</span>
                <span className="font-medium">
          ...
          <div className="grid gap-3 grid-cols-2">
            ...
            >
              {isProcessing || isCreating ? 'Processing...' : 'Request to Book'}
            </button>
          </div>
        </aside>
      </main>
